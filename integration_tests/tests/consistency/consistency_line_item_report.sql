...
with prod as (
    ...
    from {{ target.schema }}_twitter_ads_prod.twitter_ads__line_item_report
    group by 1
),
...
dev as (
    select
        ...
        sum(spend_micro) as spend_micro,
        sum(url_clicks) as url_clicks
    from {{ target.schema }}_twitter_ads_dev.twitter_ads__line_item_report
    group by 1
),
...
final as (
    ...
    from prod
    full outer join dev 
        on dev.line_item_id = prod.line_item_id
)
