{{ config(
    ...
    enabled=var('fivetran_validation_tests_enabled', false) and var('twitter_ads__using_campaign_locations_report', false)
) }}
...
model as (
    ...
    from {{ ref('twitter_ads__campaign_country_report') }}
    group by 1,2
),
